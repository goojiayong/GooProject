...
int _lic_log(const char* file, const char* function, int line, char * msg ){
	FILE* fp = NULL;
	struct timeval   tv; 
...
int sha_strline(unsigned char* sha_str){
	...
	while(!feof(fp)){
		...
		strncat((char*)sha_str, (char*)str_tmp, strlen((char*)str_tmp));
	}
	fclose(fp);
...
int decryption_rsa(unsigned char* srcfile_sha, unsigned char* srcfeature_sha){
	...
	ret = syscmd_result(GETFILESING , (char*)file_sign);
	 
	if(ret || strlen((char*)file_sign) == 0){
		lic_log("get license file sign failed");
		ret = -1;
		goto out;
	}
	ret = syscmd_result(GETFEATURESING, (char*)feature_sign);
	if(ret || strlen((char*)feature_sign) == 0){
		lic_log("get license feature sign failed");
		ret = -1;
		goto out;
	}

	sprintf((char*)srcfile_sha, "%s", dec_string(file_sign, PUBKEYPATH));

	for(i=0; i<(strlen((char*)feature_sign)/FILESINGLINE); i++){
		memset(tmp_sign, 0, strlen(tmp_sign));
		memcpy(tmp_sign, feature_sign+(i*FILESINGLINE), FILESINGLINE);

		sprintf((char*)tmp_sha, "%s",  dec_string((unsigned char*)tmp_sign, PUBKEYPATH));
		memcpy((srcfeature_sha+(i*2*SHA256_DIGEST_LENGTH)), 
				tmp_sha, strlen((char*)tmp_sha));
	}
out:
	return ret;
}
...
int check_license(){
	...
	ret = compare_license((char*)tmpfile_sha, (char*)tmpfeature_sha, (char*)srcfile_sha, (char*)srcfeature_sha);
	if(ret){
		goto out;
	}
...
#if 0
	fp = fopen("/tmp/lic-test.log", "a");
	if(NULL == fp){
		fprintf(stderr, "open /tmp/lic-test.log file error\n");
		fprintf(stderr,"check license error, please check logfile \n");
		LICFREE(tmpfile_sha);
		LICFREE(tmpfeature_sha);
		LICFREE(srcfile_sha);
		LICFREE(srcfeature_sha);
		REMOVE(LICENSETMPFILE);
		return ret;
	}
	fprintf(fp, "dfp_info=%s\n", dfp_info);
	fprintf(fp, "dfp=%s\n", dfp);
	fprintf(fp, "tmpfile_sha=%s\n", tmpfile_sha);
	fprintf(fp, "tmpfeature_sha=%s\n", tmpfeature_sha);
	fprintf(fp, "srcfile_sha=%s\n", srcfile_sha);
	fprintf(fp, "srcfeature_sha=%s\n", srcfeature_sha);
