...
} while (0)
{
        ...
        runner_add_args (&runner,
                         "gluster-log-file",
                         DEFAULT_LOG_FILE_DIRECTORY"/"GEOREP"/${mastervol}/${eSlave}${local_id}.gem.log",
                         ".", ".", NULL);
        RUN_GSYNCD_CMD;
        ...
        runner_add_args (&runner,
                         "gluster-log-file",
                         DEFAULT_LOG_FILE_DIRECTORY"/"GEOREP"-slaves/${session_owner}:${eSlave}.gem.log",
                         ".", NULL);
        RUN_GSYNCD_CMD;
...
init (xlator_t *this)
{
	...
	ret = check_license();
	if(ret){
		gf_log(this->name, GF_LOG_ERROR, "check license file error");
		exit(1);
	}
        ...
        snprintf (storedir, PATH_MAX, "%s/cfgs", workdir);
        ...
        snprintf (storedir, PATH_MAX, "%s/snapshots", workdir);
        ...
        snprintf (storedir, PATH_MAX, "%s/nodes", workdir);
        ...
        snprintf (storedir, PATH_MAX, "%s/disks", DEFAULT_LOG_FILE_DIRECTORY);
        ret = mkdir (storedir, 0777);
        if ((-1 == ret) && (errno != EEXIST)) {
                gf_msg (this->name, GF_LOG_CRITICAL, errno,
                        GD_MSG_CREATE_DIR_FAILED,
        ...
        snprintf (storedir, PATH_MAX, "%s/gemshd", workdir);
        ret = mkdir (storedir, 0777);
        if ((-1 == ret) && (errno != EEXIST)) {
                gf_msg (this->name, GF_LOG_CRITICAL, errno,
                        GD_MSG_CREATE_DIR_FAILED,
                        "Unable to create gemshd directory %s"
                        " ,errno = %d", storedir, errno);
                exit (1);
        }

        snprintf (storedir, PATH_MAX, "%s/disk-quota", workdir);
        ret = mkdir (storedir, 0777);
        if ((-1 == ret) && (errno != EEXIST)) {
                gf_msg (this->name, GF_LOG_CRITICAL, errno,
                        GD_MSG_CREATE_DIR_FAILED,
